...
/**
 * Table 'object' that converts 2d arrays to suitable HTML tables to display nice
 * @param props -takes in some 2D array as well as an indication of headers
 * @returns - HTML table
 */
export function Table(props: TableProps) {
  ...
  if (props.headers) {
    ...
    return (
      <table>
        <thead>{headerCreator(props.array[0])}</thead>
        <tbody>{tableCreator(noHeaders, 1)}</tbody>
      </table>
    );
    ...
    return (
      <table>
        <tbody>{tableCreator(props.array, 0)}</tbody>
      </table>
    );
  }
}
...
function tableCreator(array: [][], offset: number) {
  ...
  const rows = array.map(function (item, i) {
    ...
    return <tr key={i + offset}> {entry} </tr>;
  });
  return rows;
}
